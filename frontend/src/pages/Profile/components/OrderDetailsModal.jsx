        ...
        <div className="order-details-modal-overlay" onClick={onClose}>
            ...
            >
                <div className="order-details-modal-header">
                    <p className="order-details-modal-title">Заказ №{order.order_id}</p>
                ...
                <div className="order-details-modal-info-group">
                    <div className="order-details-info-row">
                        ...
                        <span className={`order-details-info-value status-${order.statusCode}`}>
                            {order.status} {order.statusDate && `(${order.statusDate})`}
                        </span>
                    </div>
                    <div className="order-details-info-row">
                        <span className="order-details-info-label">Дата оформления:</span>
                        <span className="order-details-info-value">{order.order_date}</span>
                    </div>
                    <div className="order-details-info-row">
                        <span className="order-details-info-label">Сумма заказа:</span>
                        <span className="order-details-info-value amount">{order.total_amount} ₽</span>
                    </div>
                </div>
                ...
                <div className="order-details-products-grid">
                    {Array.from({ length: order.productsCount || 2 }).map((_, index) => (
